...
module Formatter() : FORMATTER =
  struct
    ...
    let rec Spaces' arg__0 arg__1 =
      ...
      | (0, s) -> s
      | (n, s) -> Spaces' (n - 1) (s ^ " ") end
    let rec Spaces n = if n > 0 then begin Spaces' n "" end else begin "" end
let rec Newlines' arg__2 arg__3 =
  begin match (arg__2, arg__3) with
  | (0, s) -> s
  | (n, s) -> Newlines' (n - 1) (s ^ "\n") end
let rec Newlines n = if n > 0 then begin Newlines' n "" end else begin "" end
let Sp = Spaces
let rec Spmod n = Spaces (n mod_ !Pagewidth)
let Nl = Newlines
let rec Np () = "\n\012\n"
let rec Max (x, y) = if (x : int) > y then begin x end else begin y end
...
let rec Width0 =
  ...
  | (m, b, i, Hov (((min, max), _), _, _, _, _)) -> (min, max) end
...
let rec summaxwidth l =
  List.foldr
    (begin function
     | (fmt, ysum) ->
         let (_, y) = Width0 (Hori, Unused, Unused, fmt) in y + ysum end)
  0 l
let rec gh =
  begin function
  | ([], [], _) -> []
  ...
  | (cg, h::t, res) -> gh ((cg @ [h]), t, res) end
let rec pphv =
  begin function
  | (mw, li, bl, is, ss, mp, ch, lb, [], res) -> ((Max (mp, ch)), res)
  | (mw, li, bl, is, ss, mp, ch, lb, (gpwdth, flist, brk)::t, res) ->
      let (ch1, s1, mp) =
        ((if
            (lb = Ebk) ||
              ((((li + ch) + (fst (Width0 (Hori, bl, Unused, lb)))) + gpwdth)
                 <= mw)
          then
            begin let (n, s) = print'p (mw, li, bl, is, ss, Hori, lb, res) in
                  ((ch + n), s, mp) end
        else begin
...
((pphv (mw, li, bl, is, ss, mp, (ch1 + n2), brk, t, s2))
  ...
  (* Now print rest of horizontal-vertical box *)) end
let rec ppv =
  begin function
  | (mw, li, ci, bl, is, ss, max, gw, [], res) -> ((Max (max, gw)), res)
  ...
  | (mw, li, ci, bl, is, ss, max, gw, h::t, res) ->
      ...
      ppv (mw, li, (ci + n), bl, is, ss, max, (gw + n), t, s) end
let rec pph =
  begin function
  | (mw, id, bl, is, ss, [], nres, sres) -> (nres, sres)
  | (mw, id, bl, is, ss, h::t, nres, sres) ->
      let (n, s) = print'p (mw, id, bl, is, ss, Hori, h, sres) in
      pph (mw, (id + n), bl, is, ss, t, (n + nres), s) end
let rec print'p =
  begin function
  | (mw, id, bl, is, ss, mo, Str (n, s), res) -> (n, (s :: res))
  | (mw, id, bl, is, ss, Hori, Brk (b, i), res) ->
      (b, ((if !Bailout then begin Spmod b end else begin Sp b end) :: res))
  | (mw, id, bl, is, ss, Vert, Brk (b, i), res) ->
      (i,
        (((if !Bailout then begin Spmod (id + i) end
            else begin Sp (id + i) end) :: (Nl ss))
      :: res))
| (mw, id, bl, is, ss, Hori, Dbk, res) ->
    (bl, ((if !Bailout then begin Spmod bl end else begin Sp bl end) :: res))
| (mw, id, bl, is, ss, Vert, Dbk, res) ->
    (is,
      (((if !Bailout then begin Spmod (id + is) end
          else begin Sp (id + is) end) :: (Nl ss))
    :: res)) | (mw, id, bl, is, ss, mo, Ebk, res) -> (0, res)
| (mw, id, bl, is, ss, mo, Hbx ((min, max), blanks, l), res) ->
    if
      !Bailout &&
        (((id + min) >= mw) && ((!) ((>=) (id mod_ !Pagewidth)) BailoutSpot))
    then
      begin pph
              ((mw + !Pagewidth), (mw + !BailoutIndent), blanks, is, ss, l,
                0, ((Nl ss) :: res)) end
    else begin pph (mw, id, blanks, is, ss, l, 0, res) end
| (mw, id, bl, is, ss, mo, Vbx ((min, max), indent, skip, l), res) ->
    if
      !Bailout &&
        (((id + min) >= mw) && ((!) ((>=) (id mod_ !Pagewidth)) BailoutSpot))
    then
      begin let id = mw + !BailoutIndent in
            ppv
              ((mw + !Pagewidth), id, id, bl, indent, skip, 0, 0, l,
                ((Nl ss) :: res)) end
    else begin ppv (mw, id, id, bl, indent, skip, 0, 0, l, res) end
    ...
    then
      begin pphv
              ((mw + !Pagewidth), (mw + !BailoutIndent), blanks, indent,
                skip, 0, 0, Ebk, gl, ((Nl ss) :: res)) end
      else begin pphv (mw, id, blanks, indent, skip, 0, 0, Ebk, gl, res) end
| (mw, id, bl, is, ss, mo, Hov
   (((min, max), (nmode, xmode)), blanks, indent, skip, l), res) ->
    if max <= (mw - id)
    then
      begin (if xmode = Hori
             then begin pph (mw, id, blanks, is, ss, l, 0, res) end
      else begin ppv (mw, id, id, blanks, indent, skip, 0, 0, l, res) end) end
else begin
  if
    !Bailout &&
      (((id + min) >= mw) && ((!) ((>=) (id mod_ !Pagewidth)) BailoutSpot))
           ...
           then
             begin pph
                     ((mw + !Pagewidth), (mw + !BailoutIndent), blanks, is,
                       ss, l, 0, ((Nl ss) :: res)) end
    else begin
      (let id = mw + !BailoutIndent in
       ppv
         ((mw + !Pagewidth), id, id, blanks, indent, skip, 0, 0, l,
           ((Nl ss) :: res))) end) end
else begin
  if nmode = Hori then begin pph (mw, id, blanks, is, ss, l, 0, res) end
  else begin ppv (mw, id, id, blanks, indent, skip, 0, 0, l, res) end end end end
let rec makestring_fmt fm =
  String.concat
    (rev
       (snd (print'p (!Pagewidth, 0, !Blanks, !Indent, !Skip, Hori, fm, []))))
let rec print_fmt fm = List.foldr (begin function | (s, _) -> print s end) ()
  (snd (print'p (!Pagewidth, 0, !Blanks, !Indent, !Skip, Hori, fm, [])))
type fmtstream =
...
let rec output_fmt (Formatstream outs, fm) =
  List.foldr (begin function | (s, _) -> TextIO.output (outs, s) end) ()
  (snd (print'p (!Pagewidth, 0, !Blanks, !Indent, !Skip, Hori, fm, [])))
let rec file_open_fmt filename =
  let fmt_stream = open_fmt (TextIO.openOut filename) in
  let close_func =
    begin function | () -> TextIO.closeOut (close_fmt fmt_stream) end in
  (close_func, fmt_stream)
let rec with_open_fmt filename func =
  let (close_func, fmt_stream) = file_open_fmt filename in
  let result =
    begin try func fmt_stream
    with | exn -> (begin close_func (); raise exn end) end in
begin close_func (); result end end



module Formatter : FORMATTER = (Formatter)(struct  end) 