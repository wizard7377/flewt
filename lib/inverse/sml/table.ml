 ...
 *)
    ...
    val table :
      int ->
        (('a)(** Give the expected size of the eventual table. 
   ...
   *))
    ...
    val insert :
      'a table ->
        (key * 'a) ->
          (('a)(** Insert a key/value pair into the table.  
      Raise Fail if key is already present. *))
            table
    val lookup :
      'a table ->
        key ->
          (('a)(** Lookup a key in the table. Raise Fail if key not present *))
    val size :
      'a table -> ((int)(** number of key/value pairs *))
    val app :
      ('a -> unit) ->
        'a table ->
          ((unit)(** Iterate a procedure over the table. *))
    val appi :
      ((int * 'a) -> unit) ->
        'a table ->
          ((unit)(** Iterate a procedure over the table. *))
    val clear : 'a table -> ((unit)(** Clear the table. *))
  end;;
...
module ArrayTable : TABLE =
  struct
    module L =
      ((Lib)(* structure GrowarrayTable : TABLE where type key = int = *)
      (* struct *)(*   structure L = Lib *)
      (*   structure A = GrowArray *)(*   type key = int  *)
      (*   type 'a table = 'a A.growarray *)(*   fun empty _ = A.empty() *)
      (*   fun size t = A.length t *)(*   fun insert t (n,v) = *)
      (*       if A.length t > n then raise Fail "insert: signat contains key" *)
      (*       else (A.update t n v;t) *)(*   fun lookup t n = A.sub t n *)
