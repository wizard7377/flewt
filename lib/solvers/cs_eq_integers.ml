...
module CSEqIntegers(CSEqIntegers:sig
  ...
  *)
    ...
    let rec delaySum (__g, sum) =
      ...
      let cnstr = ref (Eqn (__g, __u, (numberExp zero))) in Delay (__u, cnstr)
    let rec solveSum =
      function
      | (__g,
         (Sum (m, (Mon (n, ((EVar (r, _, _, _) as x), s)::[]))::[]) as sum))
          ...
          if Whnf.isPatSub s
          then
            [Assign (__g, x, (numberExp (~- (quot (m, n)))), (Whnf.invert s))]
           ...
           | Some (Mon (n1, (X1, s1)::[]), ss1, sum1) ->
               (match findMon invertMon (__g, sum1) with
                | Some (Mon (n2, (X2, s2)::[]), ss2, sum2) ->
                    ...
                    let K = newEVar (__g', (number ())) in
                    let Z = newEVar (__g', (number ())) in
                    (::) ((::) (Assign
                                  (__g, X1,
                                    (toFgn
                                       (plusSum
                ...
                | None ->
                    (match divideSum (sum1, n1) with
                     | Some sum1' ->
                         [Assign (__g, X1, (toFgn (unaryMinusSum sum1')), ss1)]
    ...
    let rec unifySum (__g, sum1, sum2) =
      ...
      | sum -> if solvableSum sum then Succeed (solveSum (__g, sum)) else Fail
    let rec toFgn =
      function
    ...
    *)
    (* delaySum (__g, sum) = Delay (__u, cnstr)
       where __u the foreign expression corresponding to sum
       and cnstr is the constraint __g |- sum = 0 : integer
    *)
