module type CS_EQ_FIELD  =
  sig
    ...
    type sum_ =
      | Sum of (Field.number * mon_ mset) 
    and mon_ =
      | Mon of (Field.number * (IntSyn.exp_ * IntSyn.sub_) mset) 
...
let rec unifySum (g_, sum1, sum2) =
  let rec invertMon =
    begin function
    | (g_, Mon (n, ((EVar (r, _, _, _) as LHS), s)::[]), sum) ->
        if Whnf.isPatSub s
        then
...
let rec init (cs, installF) =
  ...
  (:=) numberID installF
    ((ConDec
        (Field.name, None, 0, (Constraint (!myID, solveNumber)), (Uni Type),
          Kind)), None, [MS.Mnil]);
