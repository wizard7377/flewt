...
module type RECON_MODULE  =
  sig
    ...
    val structdecToStructDec :
      (structdec * ModSyn.module_ option) -> structDec_
...
module ReconModule(ReconModule:sig
  ...
  struct
    ...
    let rec strexp (ids, id, r) () =
      ...
      | Some mid -> (mid, r) end
    let rec strexpToStrexp (f : strexp) = (fun r -> r.1) (f ())
    type inst_ =
    ...
    let rec coninst ((ids, id, r1), tm, r2) (ns, eqns) =
      ...
      | Some cid -> (((cid, (External tm), r2) :: eqns)
          (* this is wrong because constants in the sig being instantiated might incorrectly appear in tm -kw *)) end
  let rec addStructEqn (rEqns, r1, r2, ids, mid1, mid2) =
    ...
    let rec doConst (name, cid1) =
      ...
      | Some cid2 -> push (cid1, (Internal cid2), r2) end in
      ...
      | None ->
          error
            (r1,
              ((^) "Instantiating structure lacks component "
                 Names.qidToString (Names.Qid ((rev ids), name))))
      | Some mid2 -> addStructEqn (rEqns, r1, r2, (name :: ids), mid1, mid2) end in
    begin Names.appConsts doConst ns1; Names.appStructs doStruct ns1 end
let rec strinst ((ids, id, r1), strexp, r3) (ns, eqns) =
  ...
  let mid1 =
    ...
    | Some mid1 -> mid1 end in
  let (mid2, r2) = strexp () in
  let rEqns = ref eqns in
  begin addStructEqn (rEqns, r2, r3, [], mid1, mid2); !rEqns end
...
let rec structdec (idOpt, sigexp) moduleOpt =
  let (module_, inst) = sigexp moduleOpt in StructDec (idOpt, module_, inst)
             ...
             raise
               (ExtSyn.Error
                  ((^) (msg ^ "\nin instantiation generated for ")
                     Names.qidToString (Names.constQid cid))) end)
    | ((External tm, r), condec) ->
        ModSyn.strictify (ExtSyn.externalInst (condec, tm, r)) end in
  let rec transformConDec (cid, condec) =
    begin match IntTree.lookup table cid with
    | None -> condec
    | Some { contents = l } -> List.foldr doInst condec l end in
  transformConDec
let rec moduleWhere (module_, wherecl) =
  ...
  let _ = Names.resetFrom (mark, markStruct) in ((module')
    (* val _ = IntSyn.resetFrom (mark, markStruct) *)) end
