...
module type SOLVE  =
  sig
    ...
    val solve :
      ExtQuery.define list ->
        ExtQuery.solve ->
          Paths.location -> (IntSyn.__ConDec * Paths.occConDec option) list
    val query :
  ...
  struct
                ...
                (try
                   Timers.time Timers.ptrecon PtRecon.solve
                     (Skel, (g, IntSyn.id),
                       ...
                       (fun (Skel) -> fun (__M) -> raise (Solution __M)));
                   raise
                     (AbortQuery "Proof reconstruction for %solve failed")
                 with | Solution (__M) -> finish __M)
              with
              | TimeLimit.TimeOut ->
                  raise
                    (AbortQuery "\n----------- TIME OUT ---------------\n"))))
      ...
      let rec scInit (__M) =
        ...
        (match optName with
         ...
         | Some name ->
             ...
             then
               Msg.message
                 ((Timers.time Timers.printing evarInstToString [(__M, name)])
                    ^ "\n")
             else ());
        ((if (!Global.chatter) >= 3
          then
        ...
        (match optName with
         ...
         | Some name ->
             (if (!Global.chatter) > 3
              then
                (Msg.message "\n pskeleton \n";
                 Msg.message ((CompSyn.pskeletonToString __M) ^ "\n"))
              else ();
              Timers.time Timers.ptrecon PtRecon.solve
                (__M, (g, IntSyn.id),
                  (CompSyn.DProg (IntSyn.Null, IntSyn.Null)),
    ...
    let rec qLoops' =
      ...
      | Cons (query, s') ->
          ...
          let g =
            Timers.time Timers.compiling Compile.compileGoal
              (IntSyn.Null, __A) in
          let rec scInit (__M) =
            if (!Global.chatter) >= 1
            then
