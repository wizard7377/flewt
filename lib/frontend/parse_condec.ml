...
module ParseConDec(ParseConDec:sig
  ...
  struct
    ...
    let rec parseConDec2 =
      ...
      | (None, (tm, Cons ((t, r), s'))) ->
          Parsing.error (r, "Illegal anonymous declared constant") end
  ...
  let rec parseBlock =
    ...
    | Cons ((t, r), s') ->
        Parsing.error (r, ((^) "Expected `block', found " L.toString t)) end
let rec parseSome =
  begin function
  ...
  | (name, Cons ((t, r), s')) ->
      Parsing.error
        (r, ((^) "Expected `some' or `block', found " L.toString t)) end
...
let rec parseBlockDec' =
  ...
  | Cons ((t, r), s') ->
      Parsing.error
        (r, ((^) "Label identifier expected, found token " L.toString t)) end
let rec parseConDec' =
  begin function
  | Cons ((ID (idCase, name), r), s') ->
...
let parseClause' = parseClause' end
