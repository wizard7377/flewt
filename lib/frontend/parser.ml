module type PARSER  =
  sig
    ...
    type fileParseResult =
      ...
      | NamePref of ((Names.qid_ * Paths.region) * (string list * string
      list)) 
...
module Parser(Parser:sig
  ...
  struct
    ...
    let rec parseBound' =
      ...
      | Cons ((ID (_, name), r), s') ->
          (begin try ((Some (L.stringToNat name)), s')
           with | Overflow -> Parsing.error (r, "Bound too large")
           | NotDigit _ ->
               Parsing.error
                 (r,
                   (("Bound `" ^ name) ^ "' neither `*' nor a natural number")) end)
      | Cons ((t, r), s') ->
          Parsing.error
            (r,
...
let rec recParse (s, recparser, theSigParser, sc) =
  Stream.delay
    (begin function
     | () -> recParse' ((LS.expose s), recparser, theSigParser, sc) end)
let rec recParse' (f, recparser, theSigParser, sc) =
  ...
  | (Continuation k, Cons ((L.LBRACE, r1), s')) ->
      let rec finish =
        ...
        | Cons ((t, r), _) ->
            Parsing.error (r, ((^) "Expected `}', found " L.toString t)) end in
    Stream.Cons ((BeginSubsig, r1), (theSigParser (s', finish)))
  | (Continuation _, Cons ((t, r), _)) ->
      Parsing.error (r, ((^) "Expected `{', found " L.toString t)) end
let rec parseStream (s, sc) =
  Stream.delay (begin function | () -> parseStream' ((LS.expose s), sc) end)
let rec parseStream' =
  begin function
  | ((Cons ((ID (idCase, name), r0), s') as f), sc) -> parseConDec' (f, sc)
  ...
  | (Cons ((t, r), s'), sc) ->
      Parsing.error
        (r,
          ((^) "Expected constant name or pragma keyword, found " L.toString
             t)) end(* -fp *)(* -ABP 4/4/03 *)
(* -rv *)(* -bp *)(* -bp *)(* -bp *)
(* -cs *)(* -fp *)
...
let rec parseTrustMe' ((Cons ((_, r0), s) as f), sc) =
  let rec parseNextDec' =
    ...
    | Stream.Empty -> Parsing.error (r0, "No declaration after `%trustme'") end in
...
let rec parseSubord' ((Cons ((_, r0), s) as f), sc) =
  ...
  let qidpairs =
    map
      (begin function | (qid1, qid2) -> ((Names.Qid qid1), (Names.Qid qid2)) end)
    qidpairs in
...
let rec parseThaw' ((Cons ((_, r0), s) as f), sc) =
  let (qids, (Cons ((_, r'), _) as f')) = ParseTerm.parseThaw' (LS.expose s) in
  let r = Paths.join (r0, r') in
  let qids = map Names.Qid qids in
...
let rec parseUse' =
  ...
  | (Cons ((_, r), _), sc) ->
      Parsing.error (r, "Constraint solver name expected") end
let rec parseQ s =
  Stream.delay (begin function | () -> parseQ' (LS.expose s) end)
let rec parseQ' f =
...
let rec parseTLStream instream =
  let rec finish =
    ...
    | Cons ((L.RBRACE, r), s) -> Parsing.error (r, "Unmatched `}'") end in
parseStream ((L.lexStream instream), finish)
let parseStream = parseTLStream
let rec parseTerminalQ prompts = parseQ (L.lexTerminal prompts) end
