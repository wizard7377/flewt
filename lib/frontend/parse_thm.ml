...
module ParseThm(ParseThm:sig
  ...
  struct
    ...
    let rec idToNat (r, name) =
      begin try L.stringToNat name
      with | Overflow -> Parsing.error (r, "Integer too large")
      | NotDigit _ -> Parsing.error (r, "Identifier not a natural number") end
    let rec stripRParen =
      begin function
  ...
  let rec decideRBrace =
    ...
    | (r0, (order, Cons ((t, r), _))) ->
        Parsing.error
          ((P.join (r0, r)), ((^) "Expected `}', found " L.toString t)) end
...
let rec parseArgPat =
  ...
  | Cons ((L.UNDERSCORE, r), s') ->
      let (idOpts, f') = parseArgPat (LS.expose s') in ((None :: idOpts), f')
  | f -> ([], f) end
let rec parseCallPat =
  begin function
  | Cons ((ID (_, id), r), s') ->
      let (idOpts, (Cons ((_, r'), _) as f')) = parseArgPat (LS.expose s') in
      ((id, idOpts, (P.join (r, r'))), f')
  | Cons ((t, r), s) ->
      Parsing.error
        (r, ((^) "Expected call pattern, found token " L.toString t)) end
let rec parseCallPats =
  ...
  | Cons ((L.LPAREN, r), s') ->
      ...
      ((cpat :: cpats), f'')
  | Cons ((L.DOT, r), s') as f -> ([], f)
  | Cons ((t, r), s) ->
      Parsing.error
        (r, ((^) "Expected call patterns, found token " L.toString t)) end
(* Parens around call patterns no longer optional *)
let rec parseOrderOpt =
  begin function
  | Cons ((L.LPAREN, r), s') -> decideRParen (r, (parseIds (LS.expose s')))
  | Cons ((L.LBRACE, r), s') ->
...
let rec parseOrder' =
  ...
  | (None, Cons ((t, r), s')) ->
      Parsing.error (r, ((^) "Expected order, found " L.toString t)) end
...
let rec parsePDecl =
  ...
  | Cons ((ID (_, id), r), s') ->
      ...
      let (t', f') = parseTDecl (LS.expose s') in ((E.prove (depth, t')), f')
  | Cons ((t, r), s') ->
      Parsing.error
        (r, ((^) "Expected theorem identifier, found " L.toString t)) end
...
let rec parseDec (r, f) =
  ...
  let dec =
    ...
    | Some y -> E.ExtSyn.dec (x, y, (P.join (r, r2))) end in
  (dec, f'')
let rec parseDecs' =
  begin function
...
let rec parsePi =
  ...
  | Cons ((t, r), s') ->
      Parsing.error (r, ((^) "Expected `pi', found " L.toString t)) end
let rec parseSome =
  begin function
  ...
  | (gbs, Cons ((t, r), s')) ->
      Parsing.error (r, ((^) "Expected `some' or `pi', found " L.toString t)) end
let rec parseSome' =
  begin function
  | (gbs, (Cons ((L.RPAREN, r), s') as f)) -> (gbs, f)
...
let rec parseGBs =
  begin function
  | Cons ((L.LPAREN, r), s') -> stripParen (parseSome ([], (LS.expose s')))
  | Cons ((t, r), s') ->
      Parsing.error (r, ((^) "Expected `(', found " L.toString t)) end
...
let rec parseTrue =
  ...
  | Cons ((t, r), s') ->
      Parsing.error (r, ((^) "Expected `true', found " L.toString t)) end
let rec parseExists =
  begin function
  ...
  | Cons ((t, r), s') ->
      Parsing.error
        (r, ((^) "Expected `exists' or `true', found " L.toString t)) end
let rec parseForall =
  begin function
  ...
  | Cons ((t, r), s') ->
      Parsing.error
        (r,
          ((^) "Expected `forall', `exists', or `true', found " L.toString t)) end
...
let rec parsePredicate =
  ...
  | Cons ((L.EQUAL, r), s') -> ((E.predicate ("EQUAL", r)), (LS.expose s'))
  | Cons ((t, r), s') ->
      Parsing.error
        (r,
          ((^) "Expected reduction predicate <, = or <=, found " L.toString t)) end
...
let rec parseWorlds' (Cons ((L.WORLDS, r), s')) = parseWDecl (LS.expose s')
...
let parseWorlds' = parseWorlds' end
