...
module Timing : TIMING =
  struct
    ...
    let rec time (_, counters) (f : 'a -> 'b) (x : 'a) =
      ...
      let result = begin try Value (f x) with | exn -> Exception exn end in
  ...
  let rec sumToString (name, centers) =
    let rec sumup =
      ...
      | ((_, { contents = (c_, r_) })::centers, (CPUTime, realTime)) ->
          sumup (centers, ((plus (CPUTime, c_)), (Time.(+) (realTime, r_)))) end in
  sumup (centers, (zero, Time.zeroTime)) end 
