...
module UniqueSearch(UniqueSearch:sig
  ...
  struct
    ...
    let rec rSolve =
      function
      | (max, depth, ps', (Eq (Q), s), DProg (__g, dPool), sc, acc) ->
          if Unify.unifiable (__g, ps', (Q, s)) then sc (I.Nil, acc) else acc
