...
module FgnOpnTable(FgnOpnTable:sig
                                 type nonrec arg
                                 type nonrec result(* Author: Aleksey Kliger *)
  ...
  struct
    ...
    let rec initializeTable tbl =
      let exception CSfunNotInstalled of csid  in
        let maxCSid = ((50)(*Global.maxCSid*)) in
        let unimplemented csid =
          function | _ -> raise (CSfunNotInstalled csid) in
