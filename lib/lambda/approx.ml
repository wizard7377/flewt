...
module type APPROX  =
  sig
    type __Uni =
      | Level of
      ((int)(*! structure IntSyn : INTSYN !*)(* Author: Kevin Watkins *)
      (* Approximate language for term reconstruction *)) 
      | Next of
      ((__Uni)(* 1 = type, 2 = kind, 3 = hyperkind, etc. *))
    ...
    type __Exp =
      ...
      | CVar of ((__Exp)(* Const/Def/NSDef *)) option ref 
      | Undefined 
    val Type : __Uni
    val Kind : __Uni
    val Hyperkind : __Uni
    val varReset :
      unit ->
        ((unit)(* resets names of undetermined type/kind variables chosen for printing *))
...
module Approx(Approx:sig
                       module Whnf :
                       ((WHNF)(* Approximate language for term reconstruction *)
                       (* Author: Kevin Watkins *)(*! structure IntSyn' : INTSYN !*))
                     end) : APPROX =
  struct
    module I =
      ((IntSyn)(*! sharing Whnf.IntSyn = IntSyn' !*)
    ...
    type __Uni =
      | Level of
      ((int)(* The approximate language *)(* The approximate language is based on the idea of erasure.  The
    ...
    type __Exp =
      ...
      | CVar of ((__Exp)(* Const/Def/NSDef *)) option ref 
      | Undefined 
    let ((Type)(* Because approximate type reconstruction uses the pattern G |- U
    ...
    exception Ambiguous 
    let rec getReplacementName
      ((CVar
          ((r)(* getReplacementName (u, v, l, allowed) = name
         if u : v : l
      ...
      | NONE ->
          ...
          let try__
            ((i)(* others impossible by invariant *)) =
            let name = ((^) ("%" ^ pref) Int.toString i) ^ "%" in
            match varLookupName name with
            | NONE -> (varInsert ((U, V, L), name); name)
            | SOME _ -> try__ (i + 1) in
          try__ 1
    let rec findByReplacementName
      ((name)(* findByReplacementName (name) = (u, v, l)
    ...
    let rec uniToApx =
      function
      | ((I.Type)(* converting exact terms to approximate terms *)
          (* uniToApx (L) = L- *)) -> Type
      | I.Kind -> Kind
    let rec expToApx =
      function
     ...
     or G |- U ":" V = "hyperkind" *))
      ...
      | Root (FVar (name, _, _), _) ->
          let (((U)(* must have been created to represent a CVar *)),
               V, L)
    ...
    let rec exactToApx
      (((U)(* exactToApx (U, V) = (U-, V-)
     if G |- U : V *)),
       V)
      =
      let (V', L') = classToApx V in
      match whnfUni L' with
      | Level 1 -> (((Undefined)(* Type *)), V', L')
      | _ ->
          let (((((U')(* Kind/Hyperkind *)), _))(* V' *))
            = expToApx U in
          (U', V', L')
    let rec constDefApx
      ((d)(* constDefApx (d) = V-
    ...
    let rec apxToUniW =
      function
      | Level
          ((1)(* converting approximate terms to exact terms *)(* apxToUni (L-) = L *))
          -> I.Type
      | Level 2 -> I.Kind
    let rec apxToUni
      ((L)(* others impossible by invariant *)) =
      apxToUniW (whnfUni L)
    let rec apxToClassW =
      function
      | (((G)(* apxToClass (G, v, L-, allowed) = V
    ...
    let rec apxToClass
      (((G)(* Undefined case impossible *)), V, L, allowed)
      = apxToClassW (G, (whnf V), L, allowed)
    let rec apxToExactW =
      function
      | (((G)(* apxToExact (G, u, (V, s), allowed) = U
     if u : V-
     and G' |- V : L and G |- s : G'
     then U- = u and G |- U : V[s] and U is the most general such *)),
         U, (Pi ((D, _), V), s), allowed) ->
    ...
    exception Unify of
      ((string)(* matching for the approximate language *)) 
    let rec occurUniW =
      function
      | (((r)(* occurUni (r, l) = ()
    ...
    let rec matchUniW =
      ...
      | (Level
       ...
       effect: applies I
       otherwise raises Unify *)),
         Level i2) -> if i1 = i2 then () else raise (Unify "Level clash")
      | (Level i1, Next (L2)) ->
          if i1 > 1
    ...
    let rec occurW =
      ...
      | (((r)(* occur (r, u) = ()
       ...
       otherwise raises Unify *)),
         Arrow (V1, V2)) -> (occur (r, V1); occur (r, V2))
      | (r, CVar r') ->
          if r = r'
